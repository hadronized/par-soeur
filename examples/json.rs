...
fn value_parser<'a>() -> TopParser<'a, impl Fn(&'a str) -> Parser<'a, Value, str>, Value, str> {
  let string_parser = || {
    TopParser::from_input_parser(|input: &str| {
      ...
      let input = &input[1..];
      return Parser::Parsed { data: s, input };
    })
  };
  ...
  let ws_parser = || par_soeur::parse_while(char::is_whitespace);

  let array_el_parser = {
    value_parser()
      .left(parse_lexeme(",").left(ws_parser()))
      .many0()
  };
  let array_parser = {
    parse_lexeme("[")
